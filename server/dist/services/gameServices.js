...
const maskLettersInWord = (word) => {
    return word.replace(/[A-Za-z]/g, '_');
};
const guessLetter = (letter, game) => {
    // Add the new guess
...
const updateMaskedWord = (word, guesses) => {
    let maskedWord = '';
...
const saveAndReturnGame = async (game, isCorrect, res) => {
    await game.save();
    return res.json({
        id: game._id,
...
const handleWin = async (game, res) => {
    game.isComplete = true;
    game.isWinner = true;
